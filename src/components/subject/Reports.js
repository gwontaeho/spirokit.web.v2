...
export const ReportFVC = ({ data }) => {
    ...
    const trialsPreGraphs = trialsPre.map(({ measurementId, graph }) => ({ id: measurementId, data: graph.volumeFlow, type: "pre" }));
    ...
    return (
        <div className="absolute w-0 h-0 overflow-hidden">
            <div id="report-0" className="relative w-[1090px] h-[1682px] min-w-[1090px] min-h-[1682px] px-[30px] bg-white">
                ...
                <div className="text-center h-[38px] bg-[#DEE9FF] text-[24px] font-[700] mb-[16px]">Spirometry(BTPS) Best Data</div>
                ...
                <div className="text-center h-[38px] bg-[#DEE9FF] text-[24px] font-[700] mb-[16px]">Spirometry(BTPS) All Trials Pre Data</div>
                ...
                <div className="font-[500]">{CONDITIONS[diagnosis.condition]?.["ko"]}</div>
            </div>
        </div>
    );
};
                        ...
                        <table className="w-full h-fit text-[14px]">
                            ...
                            <tbody>
                                {TITLES_SVC.map(({ l, t }) => {
                                    const result = (bestPre.results || []).find(({ title }) => title === t) || {};
                                    return (
                                        <tr key={`report-svc-best-${t}`} className="h-[28px] [&>th]:font-[500] [&>th]:text-left [&>td]:text-center">
                                            <th>{l}</th>
                                            <td>{result.unit}</td>
                                            <td>{result.meas || "-"}</td>
                                            <td>{result.pred || "-"}</td>
